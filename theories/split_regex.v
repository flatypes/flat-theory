...
Section split_regex.
  ...
  Lemma re_split_at_index_spec s r i :
    s ∈ r →
    (0 ≤ i < length s)%nat →
    (take i s) ∈ (re_split_at_index r i).1 ∧
    (drop i s) ∈ (re_split_at_index r i).2.
  Proof.
    intros Hs ?.
  ...
  Lemma re_split_at_char_spec s c r :
    s ∈ r →
    let i := str_index_of s c in
    0 ≤ i →
    (take (Z.to_nat i) s) ∈ (re_split_at_char r c).1 ∧
    (drop (Z.to_nat i) s) ∈ (re_split_at_char r c).2.
  Proof.
    intros Hs i Hi.
    apply str_index_of_ge_0, str_find_Some in Hi as [Hi ?].
    eapply re_find_char_Some in Hi as [r1 [r2 [? [??]]]]; [|done..].
    unfold re_split_at_char. destruct (re_find_char r c) as [??]. simpl in *. split.
    all: eapply elem_of_union_many; [|done].
    all: apply elem_of_list_fmap; by exists (r1, r2).
  Qed.
